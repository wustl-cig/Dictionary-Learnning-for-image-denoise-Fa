...
for i=1:20
         ...
         GradNew = Grad(New);
         GradX = ExpatchG(n,GradNew);
         GradX = [GradX(:,:,1),GradX(:,:,2)];
        
        
         alpha=fista(alpha,GradX,Dictionary,rho,tau);
         
         
         Dictionary=fistaDG(GradX,alpha, Dictionary,rho);
         New = fistaXG(New,I,Dictionary,alpha,rho);
    
    
    fprintf("calculating"+ i);
    dcost(1,i)=sum(vecnorm(I-New).^2);
    ccost(1,i)=rho*sum(vecnorm(GradX-Dictionary*alpha).^2);
...
%     outs.snr(indIter) = evaluateSNR(x, imcrop(xhat,position));
    ...
    axis equal off tight;
    colormap gray;
    title(sprintf('noise image: %.2f dB', psnr(I, Io)));
    set(gca, 'FontSize', 18);
